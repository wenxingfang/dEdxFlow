...
def cubic_spline(
    ...
    if inverse:
        ...
        #root1_mask = ((input_left_cumwidths[three_roots_mask] - eps) < root_1).double()
        #root1_mask *= (root_1 < (input_right_cumwidths[three_roots_mask] + eps)).double()

        #root2_mask = ((input_left_cumwidths[three_roots_mask] - eps) < root_2).double()
        #root2_mask *= (root_2 < (input_right_cumwidths[three_roots_mask] + eps)).double()

        #root3_mask = ((input_left_cumwidths[three_roots_mask] - eps) < root_3).double()
        #root3_mask *= (root_3 < (input_right_cumwidths[three_roots_mask] + eps)).double()
        ...
        _, mask_index = torch.sort(masks, dim=-1, descending=True)[..., 0][..., None]##wxfang,FIXME, the argsort is not support in onnx, using sort() instead
