...
class MaskedLinear(nn.Linear):
    ...
    def _get_mask_and_degrees(
        ...
        if is_output:
            ...
            #mask = (out_degrees[..., None] > in_degrees).double()
    ...
    def forward(self, x):
        #print('MaskedLinear:forward:x=',x.type() , ',self.weight=',self.weight.type() ,',self.mask=',self.mask.type() ,',self.bias=',self.bias.type() )
...
class MADE(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(num_blocks):
            ...
            #prev_out_degrees = blocks[len(blocks)-1].degrees##wxfang, not important
